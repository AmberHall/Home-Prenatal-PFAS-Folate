...
Date last edited: November 5, 2024
...
# Removing values where the participants don't have whole blood folate measures
...
# Renaming `ME-PFOSA-ACOH2` to MeFOSAA, `ET-PFOSA-ACOH` to EtFOSAA, and PFDEA to PFDA for both the long and wide datasets for consistency with the current literature
  # Wide data set
pfas_folate <- pfas_folate %>%
  rename(MeFOSAA = `ME-PFOSA-ACOH2`, EtFOSAA = `ET-PFOSA-ACOH`, PFDA = PFDEA)

  # Long data set
pfas_long <- pfas_long %>%
  mutate(analyte_code = recode(analyte_code, `ME-PFOSA-ACOH2` = "MeFOSAA", `ET-PFOSA-ACOH` = "EtFOSAA", PFDEA = 'PFDA'))
...
This block of code: 
   ...
   2. Categorizes 
     ...
     Note. These categorical variables were used for table 2 only. They are reported this way for better interpretability. 
...
PFAS_list <- c("PFOA", "PFNA", "PFDA",
               "PFHXS", "PFOS",
               "PFOSA", "EtFOSAA", "MeFOSAA")
...
#Note. Too few PFOSA and EtFOSAA to look at continuously (missing PFOSA N=205, missing EtFOSAA N=287)
  # PFDA is missing N=13 and PFHxS is missing 1. 
...
This block of code 
   ...
   1. Log-transforms all continuous PFAS (PFOS, PFOA, PFHxS, PFNA, MeFOSAA)
   2. Categorizes EtFOSAA (<LOD, >=LOD) and PFDA (<0.2, 0.2, >0.2)

Notes. 
   a. PFAS in this dataset have already been imputed using the LOD/sqrt(2). 
   b. EtFOSAA was categorized due to a large number of non-detects; PFDA was categorized around 0.2 ng/mL (<0.2, 0.2, >0.2) due to a point mass issue (specifically, a disproportionate number of individuals had a value at 0.2. This is likely the result of the lab equipment not being specific enough to detect PFDA with greater precision.)
```{r PFAS_transformations}
# Log transforming PFAS to be used in a functionality form assessment
pfas_folate <- pfas_folate %>%
  mutate(MeFOSAA_log2 = log2(`MeFOSAA`),
        PFHXS_log2 = log2(PFHXS),
        PFNA_log2 = log2(PFNA),
        PFOA_log2 = log2(PFOA),
        PFOS_log2 = log2(PFOS))

# Categorizing EtFOSAA and PFDA variables
    # EtFOSAA
      pfas_folate$ETPFOSA_cat <- as.factor(ifelse(is.na(pfas_folate$EtFOSAA) | pfas_folate$EtFOSAA < 0.1, "0", "1"))
    # PFDA
      pfas_folate$PFDA_cat <- as.factor(ifelse(is.na(pfas_folate$PFDA) | pfas_folate$PFDA < 0.2, "<0.2", 
                                                ifelse(pfas_folate$PFDA == 0.2, "0.2", ">0.2"))
                                        )
```
...
# Creating a function to make a summary table for all categorical variable
...
# Creating summary tables for the categorical variables
...
data_num <- pfas_folate %>%
  dplyr::select(`MeFOSAA_log2`, PFHXS_log2, PFNA_log2, PFOA_log2, PFOS_log2) %>%
  dplyr::select(where(is.numeric))
data_num <- data_num %>%
  rename(`MeFOSAA` = `MeFOSAA_log2`,
    PFHxS = PFHXS_log2,
    PFNA = PFNA_log2,
    PFOA = PFOA_log2,
    PFOS = PFOS_log2)
...
# Creating a correlation matrix using the data_num set. Correlation matrix only includes complete cases (no missing values) and rounds to 2 decimal places
    #Note. As data have been imputed and there are not 'true missing', complete cases technically do not need to be specified for this data.
...
# Renaming the MeFOSAA variable
pfas_folate$MeFOSAA_log2 <- pfas_folate$`MeFOSAA_log2`
...
#Creating a data frame with all the needed output for tables
...
# Looping through the PFAS variables in my PFAS list and combining the results into a single data frame. 
...
This block of code 
  1. Runs the linear regression models for the 2 categorically analyzed PFAS (PFDA and EtFOSAA), adjusting for parity, prenatal vitamin intake, maternal race, income, maternal age, and cotinine.
``` {r Table_S1_LR_cat_adj}
# Running the linear regression models
PFDA_results <- lm(WBF2 ~ PFDA_cat + parity + prenatal_vitamin + mom_race + midincome + momagedeliv + cotinine_16w_log2, data = pfas_folate)
...
PFDA_summary <- summary(PFDA_results)
EtFOSAA_summary <- summary(EtFOSAA_results)
# Extracting confidence intervals for both models
PFDA_conf_int <- confint(PFDA_results)
EtFOSAA_conf_int <- confint(EtFOSAA_results)
# Extracting coefficient names for both models
PFDA_coef_names <- rownames(PFDA_summary$coefficients)
EtFOSAA_coef_names <- rownames(EtFOSAA_summary$coefficients)

# Filtering for relevant coefficients
  # Note. This part was necessary as PFDA had more than 2 levels
PFDA_coef_names <- grep("PFDA_cat", PFDA_coef_names, value = TRUE)
EtFOSAA_coef_names <- grep("ETPFOSA_cat", EtFOSAA_coef_names, value = TRUE)

# Putting the results into 2 data frames
PFDA_results_df <- data.frame(PFAS = PFDA_coef_names,
                               Estimate_CI = paste0(Estimate_CI = round(PFDA_summary$coefficients[PFDA_coef_names, "Estimate"], 1), 
  " (", round(PFDA_conf_int[PFDA_coef_names, 1], 1), ", ", round(PFDA_conf_int[PFDA_coef_names, 2], 1), ")"))
...
Table_S1_cat_adj <- rbind(EtFOSAA_results_df, PFDA_results_df)

# Printing the merged results
print(Table_S1_cat_adj)

# Removing items that are unnecessary
rm (PFDA_results, EtFOSAA_results, PFDA_summary, EtFOSAA_summary, PFDA_conf_int, EtFOSAA_conf_int, PFDA_coef_names, EtFOSAA_coef_names, PFDA_results_df, EtFOSAA_results_df)
```
...
# PFDA
PFDA_results <- lm(WBF2 ~ PFDA_cat, data = pfas_folate)
# EtFOSAA
...
PFDA_results_df <- data.frame(PFAS = PFDA_coef_names,
                               Estimate_CI = paste0(
                                  round(PFDA_summary$coefficients[PFDA_coef_names, "Estimate"], 1),
                                      " (", round(PFDA_conf_int[PFDA_coef_names, 1], 1), ", ", 
                                      round(PFDA_conf_int[PFDA_coef_names, 2], 1), ")"))
#Et-PFOSAA
EtFOSAA_results_df <- data.frame(PFAS = EtFOSAA_coef_names,
...
Table_S1_cat_CRUDE <- rbind(EtFOSAA_results_df, PFDA_results_df)
...
This block of code 
  1. Creates the output for Supplemental Table 1 by combining the four tables I created in the 2 code blocks above. 
``` {r Table_S1_Final}
# Combining the 2 adjusted data frames
Table_S1_adj <- rbind(Table_S1_cont_adj, Table_S1_cat_adj)

# Combining the 2 crude data frames
Table_S1_CRUDE <- rbind(Table_S1_cont_CRUDE, Table_S1_cat_CRUDE)

# Placing the 2 new data frames side by side
...
data <- data.frame(
  PFAS = factor(c("PFOS", "PFOA", "PFNA", "PFHxS", "MeFOSAA"), 
                levels = c("PFOS", "PFOA", "PFNA", "PFHxS", "MeFOSAA")),
...
data <- data.frame(
  PFAS = factor(c("PFDA (>0.2 ng/mL)", "PFDA (0.2 ng/mL)", "", "EtFOSAA (\u2265 LOD)"), 
                levels = c("PFDA (>0.2 ng/mL)", "PFDA (0.2 ng/mL)", "", "EtFOSAA (\u2265 LOD)")),
...
qgcomp.df <- data.frame(
  category = c("PFHxS", "PFNA", "MeFOSAA", "PFOS", "PFOA"),
...
# Re-coding categorical dummy variables to use for these analyses
...
This block of code 
  1. Creates a plot of the adjusted differences in whole blood folate levels by PFAS mixture quantile (ranging from 25% to 75%). 
``` {r Figure_2} 
...
Quant <- ggplot(risks.overall, aes(x = quantile, y = est, ymin = est - 1.96 * sd, ymax = est + 1.96 * sd)) +
  ...
  labs(x = "PFAS mixture quantile (percentile)", y = "Difference in whole blood folate (nmol/L)") +  
...
This block of code 
   1. Creates a plot of adjusted differences in whole blood folate for each PFAS (by quartile; i.e. ranging from 25% to 75%). 
``` {r Figure_3}
# Calculating adjusted differences in whole blood folate for each PFAS (by quartile), with other PFAS set at the median
...
fig3 <- ggplot(risks.singvar, aes(x = variable, y = est, ymin = est - 1.96 * sd, ymax = est + 1.96 * sd, col = q.fixed)) + 
  ...
  scale_y_continuous(name = "Difference in whole blood folate (nmol/L)", limits = c(-60, 60), breaks = seq(-60, 60, by = 20)) +
...
labels2 <- c(
  "MeFOSAA_log2" = "MeFOSAA associations when\n another PFAS is fixed at the 10th, 50th,\n and 90th percentile",
...
``` {r Figure_4_Legend} 
# Filtering the data for MeFOSAA for efficiency 
...
This block of code 
   1. Creates dose-response curves for each PFAS for the relationship between whole blood folate and PFAS z-scores in the BKMR model, when all other PFAS are set at the median
``` {r Figure_S4}
...
DR <- ggplot(pred.resp.univar, aes(z, est, ymin = est - 1.96*se, ymax = est + 1.96*se)) + 
    ...
    ylab("Whole blood folate (nmol/L)") +
    xlab("") +
    theme_minimal() +
    theme(
      plot.background = element_rect(fill = "white", color = NA),
...
Note. EMM for the 3 categories of the PFDA was only evaluated in comparison to the ref (<0.2)
``` {r Table_S3}
...
Table_S3_noPFDA <- process_pfas(pfas_list, pfas_folate)

# Evaluating the categorized PFDA variable:
# Running the models
# Other group model
model_Other <- lm(WBF2 ~ PFDA_cat * momrace_relev + PFDA_cat + parity + prenatal_vitamin + momrace_relev + midincome + momagedeliv + cotinine_16w_log2, data = pfas_folate)
# White group model
model_White <- lm(WBF2 ~ PFDA_cat * mom_race + PFDA_cat + parity + prenatal_vitamin + mom_race + midincome + momagedeliv + cotinine_16w_log2, data = pfas_folate)

# Create a data frame with the required values
PFDA_results <- data.frame(
  PFDA_Cat = c("=0.2", "=0.2", ">0.2", ">0.2"),
  Group = c("White, non-Hispanic", "Another Race or Ethnicity", "White, non-Hispanic", "Another Race or Ethnicity"),
  Effect_Estimate = c(
    round(model_White$coefficients['PFDA_cat0.2'], 1),
    round(model_Other$coefficients['PFDA_cat0.2'], 1),
    round(model_White$coefficients['PFDA_cat>0.2'], 1),
    round(model_Other$coefficients['PFDA_cat>0.2'], 1)),
  CI_Lower = c(
    round(confint(model_White)["PFDA_cat0.2", 1], 1),
    round(confint(model_Other)["PFDA_cat0.2", 1], 1),
    round(confint(model_White)["PFDA_cat>0.2", 1], 1),
    round(confint(model_Other)["PFDA_cat>0.2", 1], 1)),
  CI_Upper = c(
    round(confint(model_White)["PFDA_cat0.2", 2], 1),
    round(confint(model_Other)["PFDA_cat0.2", 2], 1),
    round(confint(model_White)["PFDA_cat>0.2", 2], 1),
    round(confint(model_Other)["PFDA_cat>0.2", 2], 1)),
  P_Interaction = c(NA,round(summary(model_Other)$coefficients['PFDA_cat0.2:momrace_relevOther', 'Pr(>|t|)'], 3),
    NA,round(summary(model_Other)$coefficients['PFDA_cat>0.2:momrace_relevOther', 'Pr(>|t|)'], 3)))

# Expanding the dataset to have separate columns for each category of PFDA
PFDA_results2 <- PFDA_results %>%
  pivot_wider(
    names_from = Group,
...
Table_S3 <- rbind(Table_S3_noPFDA, Table_S3_PFDA)

# Removing items that are unnecessary
rm(PFDA_results, PFDA_results2, model_Other, model_White, extract_results, process_pfas, Table_S3_noPFDA, Table_S3_PFDA)
```
...
# Evaluating the categorized PFDA variable
model <- lm(WBF2 ~ PFDA_cat * prenatal_vitamin_relev + PFDA_cat + parity + prenatal_vitamin_relev + midincome + momagedeliv + cotinine_16w_log2, data = pfas_folate)
...
PFDA_results <- data.frame(
  ...
  Effect_Estimate = c(
    round(model$coefficients['PFDA_cat0.2'], 1),
    round(model$coefficients['PFDA_cat0.2'], 1),
    round(model$coefficients['PFDA_cat>0.2'], 1),
    round(model$coefficients['PFDA_cat>0.2'], 1)),
  CI_Lower = c(
    round(confint(model)["PFDA_cat0.2", 1], 1),
    round(confint(model)["PFDA_cat0.2", 1], 1),
    round(confint(model)["PFDA_cat>0.2", 1], 1),
    round(confint(model)["PFDA_cat>0.2", 1], 1)),
  CI_Upper = c(
    round(confint(model)["PFDA_cat0.2", 2], 1),
    round(confint(model)["PFDA_cat0.2", 2], 1),
    round(confint(model)["PFDA_cat>0.2", 2], 1),
    round(confint(model)["PFDA_cat>0.2", 2], 1)),
  P_Interaction = c(NA,round(summary(model)$coefficients['PFDA_cat0.2:prenatal_vitamin_relevEvery day', 'Pr(>|t|)'], 3),
                    NA,round(summary(model)$coefficients['PFDA_cat>0.2:prenatal_vitamin_relevEvery day', 'Pr(>|t|)'], 3)
                    )
  )

# Reshaping the data set to have separate columns for each category of PFDA
PFDA_results2 <- PFDA_results %>%
...
# Selecting and renaming the columns for the final data frame
PFDA_results <- PFDA_results2 %>%
  dplyr::select(PFDA_Cat, Estimate_CI_Less, Estimate_CI_Every, `P_Interaction_Every day`) %>%
  rename("PFAS" = PFDA_Cat, "P_Interaction" = `P_Interaction_Every day`)

# Stacking the output of these 2 tables
Table_S4 <- rbind(Interaction_vitamin, PFDA_results)

# Printing the final results table
print(Table_S4)

# Removing items that are unnecessary
rm(Interaction_vitamin, PFDA_results2, pfas_list, model, PFDA_results, extract_results, process_pfas)
```
