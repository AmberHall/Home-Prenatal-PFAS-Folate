...
library("knitr")
library("kableExtra")
library("reshape2") #For the heatmap
library("gridExtra")
library("broom")
library("bkmr")
library("patchwork")
...
summary(pfas_folate$`ET-PFOSA-ACOH`)
summary(pfas_folate$`ME-PFOSA-ACOH2`)
summary(pfas_folate$PFDEA)
summary(pfas_folate$PFHXS)
summary(pfas_folate$PFNA)
summary(pfas_folate$PFOA)
summary(pfas_folate$PFOS)
summary(pfas_folate$PFOSA)
...
#Creating a summary table to describe the n, n<LOD, percent <LOD, percent>=LOD. 
pfas_summary <- pfas_long %>%
  ...
  summarize(N = n(), 
            Above_LOD = (n() - sum(flg_lod)), 
            Percent_Below_LOD = (100 - (mean(flg_lod) * 100)), 
            Below_LOD = (mean(flg_lod) * 100)
            )
...
This block of code 
    ...
    2. Categorizing Et-PFOSA-AcOH (<LOD, >=LOD) and PFDEA (<0.2, 0.2, >0.2)
...
# Looking at mean and SD for continuous variables
  ...
  print ("Means and SDs for Continuous Variables")
  #Maternal age:
    print ("Maternal Age: Mean")
    mean(pfas_folate$momagedeliv) # no missing values
    print ("Maternal Age: SD")
    sd(pfas_folate$momagedeliv)
  # Folate levels:
    print ("Folate: Mean")
    mean(pfas_folate$WBF2) # no missing values
    print ("Folate: SD")
    sd(pfas_folate$WBF2)

# Looking at number and frequency for categorical variables
    # Creating a list of category variables
    categorical_v <- c("mom_race", "midincome_cat", "cotinine_16w_cat", "prenatal_vitamin", "parity")
    
    # Creating a function to put all of the categorical variables into a table:
    table_cat <- function(data, variable) {
      # Creating a frequency and percent table functions to use in next steps
      freq_table <- table(data[[variable]])
      perc_table <- prop.table(freq_table) * 100
      # Combining frequencies and percentages into a data frame
      summary_table <- data.frame(Category = names(freq_table),
        Frequency = as.vector(freq_table),
        Percentage = round(as.vector(perc_table), 1) # rounding to 2 decimal places
        ) 
      return(summary_table)}
    
    # Creating an empty dataframe to use
    demographics_cat <- data.frame()
    # Writing a loop to feed in all the variables in the previously created categorical_v list
    for (var in categorical_v) {summary_table <- table_cat(pfas_folate, var)
      summary_table$Variable <- var  # Add a column for the variable name
      demographics_cat <- rbind(demographics_cat, summary_table)}

# Printing the results
kable(demographics_cat, format = "html", col.names = c("Variable", "Category", "Frequency", "Percentage")) %>%
  kable_styling(full_width = FALSE) %>%
  row_spec(0:nrow(demographics_cat), background = "white")

# Removing items that are unnecessary
rm(table_cat, var, summary_table)
```
...
calc_mean_sd <- function(df, cat_var, value_var) {
  df %>%
    ...
    summarize(mean = round(mean(!!sym(value_var), na.rm = TRUE), 1),
              sd = round(sd(!!sym(value_var), na.rm = TRUE), 1)) %>%
...
folate_by_cat <- bind_rows(results)
...
data_num <- pfas_folate %>%
  select(`ME-PFOSA-ACOH2_log2`, PFHXS_log2, PFNA_log2, PFOA_log2, PFOS_log2) %>%
...
# Removing items that are unnecessary
rm(pfas_folate2, data_num, corr_mat, melted_corr_mat, fig_cont )
```
This block of code 
...
PFAS <- c('MeFOSAA_log2', 'PFHXS_log2', 'PFNA_log2', 'PFOA_log2', 'PFOS_log2')
...
#Creating a dataframe with all the needed output for tables
  data.frame(PFAS = pfas_var,
    Estimate = round(summary_fit$coefficients[pfas_var, "Estimate"], 1),
    CI_Lower = round(conf_int[pfas_var, 1], 1),
    CI_Upper = round(conf_int[pfas_var, 2], 1),
    P_Value = round(summary_fit$coefficients[pfas_var, "Pr(>|t|)"], 3))}

# Looping through the PFAS variables in my PFAS list and combining the results into a single dataframe. 
linear_results_adj <- do.call(rbind, lapply(PFAS, extract_model_results, data = pfas_folate))

# Printing the results
print(linear_results_adj)
...
linear_results_CRUDE <- do.call(rbind, lapply(PFAS, extract_model_results_CRUDE, data = pfas_folate))

# Printing the results
print(linear_results_CRUDE)
...
pfdea_results_df <- data.frame(PFAS = pfdea_coef_names,
  Estimate = round(pfdea_summary$coefficients[pfdea_coef_names, "Estimate"], 1),
  CI_Lower = round(pfdea_conf_int[pfdea_coef_names, 1], 1),
  CI_Upper = round(pfdea_conf_int[pfdea_coef_names, 2], 1),
  P_Value = round(pfdea_summary$coefficients[pfdea_coef_names, "Pr(>|t|)"], 3))

etfosaa_results_df <- data.frame(PFAS = etfosaa_coef_names,
  Estimate = round(etfosaa_summary$coefficients[etfosaa_coef_names, "Estimate"], 1),
  CI_Lower = round(etfosaa_conf_int[etfosaa_coef_names, 1], 1),
  CI_Upper = round(etfosaa_conf_int[etfosaa_coef_names, 2], 1),
  P_Value = round(etfosaa_summary$coefficients[etfosaa_coef_names, "Pr(>|t|)"], 3))

# Merging the two results data frames
categorical_results_adj <- rbind(pfdea_results_df, etfosaa_results_df)

# Printing the merged results
print(categorical_results_adj)
...
categorical_results_CRUDE <- rbind(pfdea_results_df, etfosaa_results_df)

# Printing the merged results
print(categorical_results_CRUDE)
...
This block of code 
  1. Creates the forest plots to display the results from the adjusted linear regression models for all continuous variables
``` {r Figure1_cont}
...
This block of code
  1. Creates forest plots to display the results from the adjusted linear regression models for all categorical variables
``` {r Figure1_cat}
...
# Running the quantile-based g-computaiton model
    #Note. This is using default settings (looking at quartiles, not bootstrapping, etc)
qc.fitadj <- qgcomp.noboot(WBF2 ~ PFOS_log2 + PFOA_log2 + PFNA_log2 + PFHXS_log2 + MeFOSAA_log2 +
                          parity + prenatal_vitamin + mom_race + midincome + momagedeliv + cotinine_16w_log2,
                        expnms = Xnm, data=pfas_folate, family = gaussian())

# Extracting the results I am interested in from the model
estimates <- coef(qc.fitadj)
conf_intervals <- confint(qc.fitadj)
p_values <- summary(qc.fitadj)$coefficients[, "Pr(>|t|)"]

# Creating ta dataframe with the results
...
# Removing the row containing the intercept
qg_comp_results <- qg_comp_results[qg_comp_results$psi != "(intercept)", ]

# Printing the results
print(qg_comp_results)

# Removing items that are unnecessary
rm(estimates, p_values, conf_intervals, qc.fitadj, Xnm)
```
...
ggsave(filename = "/Users/amber/Desktop/PFAS and Folate/Figures/Figure3.tiff", plot = fig3, 
       device = "tiff", 
       width = 7.5, height = 4, units = "in", 
       dpi = 300)
...
create_plot <- function(data, variable) {
  ggplot(data, aes(x = z1, y = est, color = factor(quantile))) + 
    geom_smooth(stat = "identity", size = 1) +  # Thinner lines with colors
    geom_hline(yintercept = 0, linetype = "dotted", color = "black") +  # Black dotted line at y = 0
    facet_grid(variable2 ~ variable1, labeller = labeller(variable1 = labels2, variable2 = labels)) +
    scale_color_manual(values = c("red", "darkgreen", "blue")) +  # Custom color palette
    theme_minimal(base_size = 15) +  # Clean, minimal theme with larger base font size
    labs(x = "PFAS z-score\n", y = "Total folate (nmol/L)") +  # Axis labels
    ylim(-60, 80) +
    theme(
      axis.title = element_text(size = 16, color = "black"),  # Axis title style
      axis.text = element_text(size = 14, color = "black"),  # Axis text style
      strip.text = element_text(size = 16),  # Facet label style
      strip.text.y = element_text(angle = 0),  # Ensure y-axis labels are not cut off
      panel.grid.major = element_blank(),  # Remove major grid lines
      panel.grid.minor = element_blank(),  # Remove minor grid lines
      panel.border = element_rect(color = "black", fill = NA),  # Add border around panels
      legend.position = "none",  # Remove legend
      plot.margin = unit(c(1, 1, 1, 1), "cm"),  # Add margin to ensure labels are not cut off
      plot.background = element_rect(fill = "white", color = NA),
    )
}
...
# Removing items I do not need
rm (pred.resp.df, labels2, create_plot, plots, bkmr_int)
```

Creating just the legend for Figure 4
...
# Removing items I do not need
rm (dummy_plot, legend, mefosaa_data, legend_plot)
```
...
This block of code 
  1. Evaluates effect measure modification (EMM) by prenatal vitamin intake. 
...
process_pfas <- function(pfas_list, data) {
  ...
  for (pfas_var in names(pfas_list)) {
    ...
    combined_results <- rbind(combined_results, data.frame(
      ...
      P_Value_Interact = interaction_p_val,
      stringsAsFactors = FALSE))
  }
...
Interaction_race <- process_pfas(pfas_list, pfas_folate)
print(Interaction_race)
...
PFDEA_results <- PFDEA_results2 %>%
  ...
  rename(
    "Log2- PFAS" = PFDEA_Cat,
    "P-interaction" = `P_Interaction_Another Race or Ethnicity`)

# Printing the results
print(PFDEA_results)

# Removing items that are unnecessary
rm(Interaction_race, PFDEA_results2)
```
...
# Running the results through each PFAS in the PFAS list and printing them
Interaction_vitamin <- process_pfas(pfas_list, pfas_folate)
print(Interaction_vitamin)
...
# Combining the effect estimates and CIs into a single string
PFDEA_results2 <- PFDEA_results2 %>%
  mutate(`Less than daily` = paste0(`Effect_Estimate_Less than daily`, " (", `CI_Lower_Less than daily`, ", ", 
                                    `CI_Upper_Less than daily`, ")"),
         `Every day` = paste0(`Effect_Estimate_Every day`, " (", 
                                  `CI_Lower_Every day`, ", ", `CI_Upper_Every day`, ")"))

# Selecting and renaming the columns for the final data frame
PFDEA_results <- PFDEA_results2 %>%
  select(PFDEA_Cat, `Less than daily`, `Every day`, `P_Interaction_Every day`) %>%
  rename("Log2- PFAS" = PFDEA_Cat, "P-interaction" = `P_Interaction_Every day`)

# Printing the final results table
print(PFDEA_results)

# Removing items that are unnecessary
rm(Interaction_vitamin, PFDEA_results2, pfas_list)
```
